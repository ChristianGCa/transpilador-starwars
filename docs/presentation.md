...
diferentes. Executar `./sw test` e abrir o programa completo, o relatório e o C gerado.
`./sw demo` executa em sequência todos os exemplos válidos e inválidos, útil para ensaiar.
...
3. Exibir as produções de expressão, termo e fator no relatório. Comparar as ASTs
   de `2 + 3 * 4` e `(2 + 3) * 4` usando `./sw c arquivo.starwars --ast`.
4. Transpilar `examples/valid/02_complete.starwars` com `./sw c`, mostrar o C e
   executá-lo com `./sw run ... < examples/valid/02_complete.input.txt`. Explicar por que são usados `%d`, `%f`, `%g` e `%s`.
5. Executar novamente com `0` repetições e nível `1` para demonstrar o ramo alternativo.
6. Rodar os arquivos de erro léxico, sintático e semântico separadamente com `./sw check`. Mostrar
...
8. Mostrar a execução dos testes (`./sw test`) e concluir a demonstração. Informar o apoio de IA
