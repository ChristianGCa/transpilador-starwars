...
- Estrutura: `src/starwars/` (um módulo por fase), `tests/` (um arquivo por fase), `examples/`, `docs/`,
  `sw` (script de uso rápido, testado em `tests/test_script.py`), `editors/vscode/` (extensão
  do VS Code; ao criar ou mudar uma frase reservada, atualize a gramática e `phrases.json` —
  `tests/test_vscode.py` confere que todas as frases do lexer têm cor e explicação).
